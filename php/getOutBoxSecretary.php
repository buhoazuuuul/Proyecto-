<?php
  ...
  $data = $DB->query("SELECT respuesta_mens.id AS idRespuesta, reporte.prioridad, usuario.nombre, usuario.apellido, reporte.asunto AS asunto_reporte , respuesta_mens.asunto AS asunto_respuesta, respuesta_mens.fecha_hora  FROM `respuesta_mens`
                     ...
                     INNER JOIN reporte ON reporte.id = reportes_x_respuestas.reporte_id INNER JOIN usuario ON usuario.id = reportes_x_respuestas.usuario_id  WHERE reportes_x_respuestas.secretario_id  = ? ORDER BY respuesta_mens.fecha_hora DESC", array( $_POST['id_secretario']));
  $DB->closeConnection();
